...
for (i in 1:length(all.sp)) {
  ...
  if (length(idx) > 1) {
    ...
    cols_1_6_same <- all(apply(subdat[, c(1:7,12)], 2, function(x) length(unique(x)) == 1))
    
    # check columns 7–11: some rows have data, some are empty
    cols_7_11_issue <- any(apply(subdat[, 8:11], 2, function(x) {
      has_data <- x != "" & !is.na(x)
...
for (i in 1:length(all.sp)) {
  ...
  if (length(idx) > 1) {
    ...
    cols_7_10_ok <- all(apply(subdat[, 7:11], 2, function(x) {
      vals <- x[x != "" & !is.na(x)]  # keep only real data
      length(unique(vals)) <= 1       # at most one unique value
    }))
    
    # column 12 must be different
    col11_diff <- length(unique(subdat[, 12])) > 1
...
write_xlsx(conflict_dat, "../data/Final_conflicts.xlsx")
