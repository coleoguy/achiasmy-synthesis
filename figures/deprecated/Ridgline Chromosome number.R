# =========================
# 6-panel barplots: (p1|p2)/(p3|p4)/(p5|p6)
# Fix whitespace:
# - EACH panel uses its own X limits (tight to observed haploid bins)
# - EACH panel uses its own Y limits (tight to max species count)
# =========================
...
darken_hex <- function(hex, f = 0.75) {
  rgb <- grDevices::col2rgb(hex)
  grDevices::rgb(rgb[1,] * f, rgb[2,] * f, rgb[3,] * f, maxColorValue = 255)
}
...
# per-panel x/y limits
lims <- bar_dat %>%
  group_by(clade) %>%
  summarize(
    xmin = min(h, na.rm = TRUE),
    xmax = max(h, na.rm = TRUE),
    ymax = max(species_n, na.rm = TRUE),
    .groups = "drop"
  )
xmin <- setNames(lims$xmin, lims$clade)
xmax <- setNames(lims$xmax, lims$clade)
ymax <- setNames(lims$ymax, lims$clade)
...
make_bar <- function(cl, show_y = TRUE) {
  # small padding so bars don't touch the frame
  xpad <- max(1, round(0.05 * (xmax[[cl]] - xmin[[cl]] + 1)))
  ggplot(filter(bar_dat, clade == cl), aes(h, species_n)) +
    ...
    scale_x_continuous(
      limits = c(xmin[[cl]] - xpad, xmax[[cl]] + xpad),
      breaks = scales::pretty_breaks(n = 5),
      expand = c(0, 0)
    ) +
    scale_y_continuous(
      limits = c(0, ymax[[cl]] * 1.05),
...
# order to match your screenshot (edit if desired)
...
if (length(clades6) < 6) stop("Fewer than 6 clades available: ", paste(clades6, collapse = ", "))
